    ...
    loop {
        ...
        let res = match &resp_commands[0] {
            RESPData::BulkString(bulk_string) => match bulk_string.as_slice() {
                ...
                b"ECHO" => resp_commands[1].clone(),
                b"SET" => handle_set(resp_commands, &data_store).await?,
                b"GET" => handle_get(resp_commands, &data_store).await?,
...
async fn handle_set(resp_commands: Vec<RESPData>, data_store: &Store) -> anyhow::Result<RESPData> {
    ...
    // TO CHECK: why doesn't this require mut data_store?
...
async fn handle_get(resp_commands: Vec<RESPData>, data_store: &Store) -> anyhow::Result<RESPData> {
